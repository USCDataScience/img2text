...
def main(unused_argv):
    ...
    #### Change to percent of data, randomize the selection
    val_size = int(0.05*len(mscoco_train_dataset))
    train_size = len(mscoco_train_dataset) - 2*val_size
    
    val_and_eval = random.sample(mscoco_train_dataset, val_size*2)
    val_dataset = val_and_eval[0:val_size]
    test_dataset = val_and_eval[val_size:]
    train_dataset = [image for image in mscoco_train_dataset if image not in val_and_eval]
    ...
    print(len(mscoco_train_dataset))
